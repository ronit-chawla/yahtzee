...
class TotalOneNumber extends Rule {
	evalRoll = dice => {
		return (
			this.val * this.count(dice, this.val)
		);
	};
}
...
const fullHouse = new FullHouse({
	score       : 25,
	description :
		'25 points for a 3 are the same and 2 are the same (full House)'
});
...
const smallStraight = new SmallStraight({
	score       : 30,
	description :
		'30 points for a straight of 4(Large Straight)'
});
const largeStraight = new LargeStraight({
	score       : 40,
	description :
		'40 points for a straight of 5(Large Straight)'
});
...
const yahtzee = new Yahtzee({
	score       : 50,
	description :
		'50 points for yahtzee(all same)'
});
